...
#        print('--'+file)
    ...
    print('DBSCAN...')
    dbscan_label = DBSCAN(eps=3, min_samples=30).fit_predict(new_dim_points)  # eps 距离的阈值， min_sample，成为核心所需要的样本的阈值 2.5, 3
...
#    incorrect.extend(np.setdiff1d(dbscan_8[:, 2], label_8[:, 2]))
    incorrect.extend(np.setdiff1d(label_8[:,0:2][0:int(-total_spy/total_cat)], dbscan_8[:, 0:2][0:int(-total_spy/total_cat)]))
...
#    incorrect.extend(np.setdiff1d(dbscan_16[:, 2], label_16[:, 2]))
    incorrect.extend(np.setdiff1d(label_16[:, 0:2][0:int(-total_spy/total_cat)], dbscan_16[:, 0:2][0:int(-total_spy/total_cat)]))
    incorrect = np.unique(np.array(incorrect))
...
#    incorrect.extend(np.setdiff1d(dbscan_32[:, 2], label_32[:, 2]))
    incorrect.extend(np.setdiff1d(label_32[:, 0:2][0:int(-total_spy/total_cat)], dbscan_32[:, 0:2][0:int(-total_spy/total_cat)]))
    incorrect = np.unique(np.array(incorrect))
...
#    incorrect.extend(np.setdiff1d(dbscan_64[:, 2], label_64[:, 2]))
    incorrect.extend(np.setdiff1d(label_64[:, 0:2][0:int(-total_spy/total_cat)], dbscan_64[:, 0:2][0:int(-total_spy/total_cat)]))
    incorrect = np.unique(np.array(incorrect))
...
#    incorrect.extend(np.setdiff1d(dbscan_Q[:, 2], label_Q[:, 2]))
    incorrect.extend(np.setdiff1d(label_Q[:, 0:2][0:int(-total_spy/total_cat)], dbscan_Q[:, 0:2][0:int(-total_spy/total_cat)]))
    incorrect = np.unique(np.array(incorrect))
