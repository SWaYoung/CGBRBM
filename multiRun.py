...
total_spy = 12  # spy 的总数量，目前6个，三类，之后测试更多类的时候再更改
n_epochs = 10
spy_batch_size = 12  # spy的batch数量
...
#    plt.show()
    ...
    for i in range(len(label_list)):
        ...
        elif label_list[i][29:32] == '128':
            label_128.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
        elif label_list[i][29:33] == 'QPSK':
            label_Q.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
    
    label_8.append(np.append(np.copy(new_dim_points[-12]), [total_input - 12]))
    label_8.append(np.append(np.copy(new_dim_points[-11]), [total_input - 11]))        
    ...
    label_128.append(np.append(np.copy(new_dim_points[-4]), [total_input - 4]))
    label_128.append(np.append(np.copy(new_dim_points[-3]), [total_input - 3]))
...
#    print_tsne(data=label_32, label='32', color='black')
#    print_tsne(data=label_64, label='64', color='red')
#    print_tsne(data=label_128, lal='Q', color='gold')
#    plt.show()
#    time_dif = get_time_dif(start_time)
#    print("Time usage:", time_dif)bel='128', color='purple')
#    print_tsne(data=label_Q, labe
#    
...
#    assert dbscan_label[-2] == dbscan_label[-1]
    ...
    incorrect_sum.append(incorrect)
    
    correct = total_input - total_spy - incorrect_num
    accuracy.append(correct / (total_input - total_spy))
