...
offset_height = 80  # 截取图片的起始高度（从上至下）80 ex:55
offset_width = 270  # 截取图片的其实宽度（从左至右）270 ex:145
target_height = 350  # 截取图片的高度 350 ex:750
target_width = 350  # 截取图片的宽度 350 ex:950
...
label_128.append(np.append(np.copy(new_dim_points[-12]), [total_input - 12]))
label_128.append(np.append(np.copy(new_dim_points[-11]), [total_input - 11]))        
...
print_tsne(data=label_128, label='128', color='purple')
