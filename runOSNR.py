...
# %% define variables
base_data_dir = '../Data/OSNR4'  # 所有数据文件夹的根目录
...
for file in os.listdir('./pickle')[0:int(total_cat * num_each_cat / batch_size)]:  # 所有非spy输入
...
label_list = []
start = int(total_cat * num_each_cat / batch_size + total_spy / spy_batch_size)
end = int(start + total_cat * num_each_cat / batch_size)
for file in os.listdir('./pickle')[start:end]:
    with open('./pickle/' + file, 'rb') as p:
        temp = pickle.load(p)
        label_list.extend([temp[i].decode("utf-8") for i in range(batch_size)])  # 注意要decode

# get spy labels
start = int(2 * total_cat * num_each_cat / batch_size + total_spy / spy_batch_size)
for file in os.listdir('./pickle')[start:]:
    with open('./pickle/' + file, 'rb') as p:
        temp = pickle.load(p)
        label_list.extend([temp[i].decode("utf-8") for i in range(spy_batch_size)])  # 注意： 要用spy_batch_size
time_dif = get_time_dif(start_time)
print("Time usage:", time_dif)
...
print('DBSCAN...')
start_time = time.time()
dbscan_label = DBSCAN(eps=4, min_samples=50).fit_predict(new_dim_points)  # eps 距离的阈值， min_sample，成为核心所需要的样本的阈值 2.5, 3
...
for i in range(len(label_list)):
    if label_list[i][14:16] == '11':
        label_11.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
    elif label_list[i][14:16] == '12':  # 根据文件名第29位进行识别，一下同理
        label_12.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
    elif label_list[i][14:16] == '13':
        label_13.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
    elif label_list[i][14:16] == '14':
        label_14.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
    elif label_list[i][14:16] == '15':
        label_15.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
    elif label_list[i][14:16] == '16':
        label_16.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
    elif label_list[i][14:16] == '17':
        label_17.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
    elif label_list[i][14:16] == '18':
        label_18.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
    elif label_list[i][14:16] == '19':
        label_19.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
    elif label_list[i][14:16] == '20':
        label_20.append(np.append(np.copy(new_dim_points[i]), [int(i)]))
        
label_11.append(np.append(np.copy(new_dim_points[-10]), [total_input - 10]))
label_12.append(np.append(np.copy(new_dim_points[-9]), [total_input - 9]))
label_13.append(np.append(np.copy(new_dim_points[-8]), [total_input - 8]))
label_14.append(np.append(np.copy(new_dim_points[-7]), [total_input - 7]))
label_15.append(np.append(np.copy(new_dim_points[-6]), [total_input - 6]))
label_16.append(np.append(np.copy(new_dim_points[-5]), [total_input - 5]))
label_17.append(np.append(np.copy(new_dim_points[-4]), [total_input - 4]))
label_18.append(np.append(np.copy(new_dim_points[-3]), [total_input - 3]))
label_19.append(np.append(np.copy(new_dim_points[-2]), [total_input - 2]))
label_20.append(np.append(np.copy(new_dim_points[-1]), [total_input - 1]))

label_11 = np.array(label_11)
label_12 = np.array(label_12)  # 变成numpy array
label_13 = np.array(label_13)
label_14 = np.array(label_14)
label_15 = np.array(label_15)
label_16 = np.array(label_16)
label_17 = np.array(label_17)
label_18 = np.array(label_18)
label_19 = np.array(label_19)
label_20 = np.array(label_20)
time_dif = get_time_dif(start_time)
print("Time usage:", time_dif)
...
start_time = time.time()
plt.figure()
print_tsne(data=label_11, label='11', color='green')
print_tsne(data=label_12, label='12', color='dodgerblue')
print_tsne(data=label_13, label='13', color='black')
print_tsne(data=label_14, label='14', color='red')
print_tsne(data=label_15, label='15', color='purple')
print_tsne(data=label_16, label='16', color='gold')
print_tsne(data=label_17, label='17', color='orange')
print_tsne(data=label_18, label='18', color='gray')
print_tsne(data=label_19, label='19', color='blue')
print_tsne(data=label_20, label='20', color='yellow')
plt.show()
time_dif = get_time_dif(start_time)
print("Time usage:", time_dif)
...
assert dbscan_label[-12] == dbscan_label[-11]
...
for j in range(total_cat):
    ...
    if dbscan_label[-4] == j:
        dbscan_128 = np.array(temp_set.copy())
    if dbscan_label[-2] == j:
...
incorrect.extend(np.setdiff1d(dbscan_128[:, 2], label_128[:, 2]))
incorrect.extend(np.setdiff1d(label_128[:, 2], dbscan_128[:, 2]))
