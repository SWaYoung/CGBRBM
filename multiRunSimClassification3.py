...
total_spy = 96  # spy 的总数量，目前5个类，每个类16个，每个OSNR一个
n_epochs = 15
...
for times in range(cycle):
    ...
    spy_16 = np.copy(new_dim_points[-spy_each_cat*3:-spy_each_cat*2])
    ...
    dbscan_array = [dbscan_0, dbscan_1, dbscan_2]
    dbscan_centroid = [centroid_0, centroid_1, centroid_2]
    spy_array = [spy_Q, spy_8, spy_16]
    spy_centroid = [centroid_Q, centroid_8, centroid_16]# %% compute accuracy，将通过dbscan和label分类得到的类进行对比，得到算法正确率，incorrect 中包含错误识别的序号，可在label_list中寻找
    ...
    for i in range(len(label_list)):
        ...
        elif label_list[i][29:32] == '032':  # 根据文件名第29位进行识别，一下同理
    ...
    #    spy_Q_position = np.copy(new_dim_points[-spy_each_cat:])
        ...
        print_tsne(data=spy_8, label='8spy', color='green')
        print_tsne(data=spy_16, label='16spy', color='dodgerblue')
    #    print_tsne(data=spy_32_position, label='32spy', color='purple')
    #    print_tsne(data=spy_64_position, label='64spy', color='red')
        print_tsne(data=spy_Q, label='Qspy', color='gold')
        plt.show()
    time_dif = get_time_dif(start_time)
    print("Time usage:", time_dif)
